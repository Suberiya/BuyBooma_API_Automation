...
public class ClaimTests2 {
	...
	{
		Response response= UserEndPoints2.createUser(claimpayload);
	...
	public void testGetUserById()
	{
		...
		Response response= UserEndPoints2.readUser(this.claimpayload.getClaimId());
		response.then().log().all();
	...
	public void testUpdateUserById()
	{
		...
		Response response= UserEndPoints2.UpdateUser(this.claimpayload.getClaimId(), claimpayload);
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(), 200);
		...
		logger.info("***********");
		Response responseAfterUpdate= UserEndPoints2.UpdateUser(this.claimpayload.getClaimId(), claimpayload);
	...
	{
		 Response response= UserEndPoints2.DeleteUser(this.claimpayload.getClaimId());
		 Assert.assertEquals(response.getStatusCode(), 200);
